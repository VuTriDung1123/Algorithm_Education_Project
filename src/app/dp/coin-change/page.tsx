...
import { generateCoinChange } from "@/lib/search/coinChangeLogic";
import { coinChangeSnippets } from "@/lib/search/coinChangeCode";
...
export default function Page() {
  ...
  const handleRun = () => {
      runAnimation((generateCoinChange as any)([1, 2, 5], 11));
  };
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-slate-950 text-white p-4 md:p-8 relative font-sans">
      ...
      <h1 className="text-3xl font-extrabold mb-2 bg-linear-to-r from-blue-400 to-indigo-500 bg-clip-text text-transparent">Coin Change</h1>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl sticky top-6">
                ...
                <div className="p-4 bg-[#1e1e1e] overflow-x-auto min-h-[400px]">
                    <table className="w-full border-collapse font-mono text-xs md:text-sm">
                        <tbody>
                            {coinChangeSnippets.javascript.split('\n').map((line: string, i: number) => {
