...
import { generateEditDistance } from "@/lib/search/editDistanceLogic";
import { editDistanceSnippets } from "@/lib/search/editDistanceCode";
...
export default function Page() {
  ...
  const handleRun = () => {
      runAnimation((generateEditDistance as any)("horse", "ros"));
  };
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-slate-950 text-white p-4 md:p-8 relative font-sans">
      ...
      <h1 className="text-3xl font-extrabold mb-2 bg-linear-to-r from-blue-400 to-indigo-500 bg-clip-text text-transparent">Edit Distance</h1>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl sticky top-6">
                ...
                <div className="p-4 bg-[#1e1e1e] overflow-x-auto min-h-[400px]">
                    <table className="w-full border-collapse font-mono text-xs md:text-sm">
                        <tbody>
                            {editDistanceSnippets.javascript.split('\n').map((line: string, i: number) => {
