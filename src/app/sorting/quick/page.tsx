  ...
  <div className="space-y-6 text-slate-300 leading-relaxed">
    <div>
      <h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2">
        <Info size={20} className="text-cyan-400" /> 1. What is Quick Sort?
      </h3>
      <p>
        <strong>Quick Sort</strong> is a highly efficient sorting algorithm and is based on partitioning of array of data into smaller arrays. A large array is partitioned into two arrays one of which holds values smaller than the specified value, say <strong>Pivot</strong>, based on which the partition is made and another array holds values greater than the pivot value.
      </p>
    </div>
    ...
    <div>
      ...
      <ul className="list-disc pl-5 space-y-2 marker:text-yellow-500">
        <li><strong>Pick a Pivot:</strong> Choose an element (e.g., last element) as the pivot.</li>
        <li><strong>Partitioning:</strong> Reorder the array so that all elements with values less than the pivot come before the pivot, while all elements with values greater than the pivot come after it.</li>
        <li><strong>Recursion:</strong> Recursively apply the above steps to the sub-array of elements with smaller values and separately to the sub-array of elements with greater values.</li>
      </ul>
    </div>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-slate-800 p-3 rounded border border-slate-700">
          ...
          <p className="text-xs text-slate-500">Average case. Worst case is O(n²).</p>
        </div>
        <div className="bg-slate-800 p-3 rounded border border-slate-700">
          <span className="text-slate-400 text-xs uppercase font-bold">Space Complexity</span>
          <p className="text-lg font-bold text-yellow-400">O(log n)</p>
          <p className="text-xs text-slate-500">Recursion stack space.</p>
        </div>
      </div>
    </div>
  </div>
...
function QuickSortVisualizer() {
  ...
  // --- PRACTICE LOGIC (QUICK SORT) ---
  const handlePracticeDecision = (decision: 'YES_SWAP' | 'NO_NEXT') => {
    ...
    // Nếu không phải bước so sánh, bỏ qua
    if (currentData.type !== 'COMPARE') {
        handleStepForward();
        return;
    }

    const val = currentData.variables.compareVal1 || 0; // Giá trị đang xét
    const pivot = currentData.variables.compareVal2 || 0; // Pivot
    
    let isCorrect = false;
    
    // Nếu Val < Pivot => YES_SWAP (Vì ta cần swap để đưa nó về bên trái - theo Lomuto code của ta có i++ và swap)
    // Nếu Val >= Pivot => NO_NEXT
    
    const shouldSwap = val < pivot;

    if (decision === 'YES_SWAP' && shouldSwap) isCorrect = true;
    else if (decision === 'NO_NEXT' && !shouldSwap) isCorrect = true;
  ...
  const getBarColor = (index: number) => {
    ...
    // Pivot - Màu Cam/Đỏ
    if (index === variables.pivotIdx) return "bg-red-600 shadow-[0_0_15px_rgba(220,38,38,0.7)] z-10";

    // Range đang partition - Làm nổi bật, các phần khác mờ đi
    if (variables.left !== undefined && variables.right !== undefined) {
        if (index >= variables.left && index <= variables.right) {
            // Trong range, mặc định màu xanh dương đậm hơn xíu
            if (!indices.includes(index) && index !== variables.pivotIdx) return "bg-blue-600";
        } else {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-slate-950 text-white p-4 md:p-8 relative">
        ...
        {isTheoryOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={() => setIsTheoryOpen(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-slate-900 border border-slate-700 w-full max-w-2xl max-h-[80vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center p-6 border-b border-slate-800 bg-slate-950"><h2 className="text-2xl font-bold text-white flex items-center gap-2"><BookOpen className="text-purple-400" /> Theory: Quick Sort</h2><button onClick={() => setIsTheoryOpen(false)}><X size={24} /></button></div>
              <div className="p-6 overflow-y-auto custom-scrollbar"><QuickSortTheory /></div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex justify-between items-center w-full max-w-7xl mb-6">
        <button onClick={() => setIsTheoryOpen(true)} className="group flex items-center gap-3 text-3xl font-bold bg-linear-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent">Quick Sort Visualization <BookOpen className="text-slate-500 group-hover:text-purple-400" size={24} /></button>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 w-full max-w-7xl">
        <div className="xl:col-span-2 space-y-6">
            ...
            <div className="flex flex-wrap justify-center gap-6 pb-2">
                <div className="flex items-center gap-2"><div className="w-3 h-3 bg-red-600 rounded"></div><span className="text-slate-400 text-xs">Pivot</span></div>
                <div className="flex items-center gap-2"><div className="w-3 h-3 bg-blue-600 rounded"></div><span className="text-slate-400 text-xs">Partition Range</span></div>
                ...
                <div className="flex items-center gap-2"><div className="w-3 h-3 bg-orange-500 rounded"></div><span className="text-slate-400 text-xs">Swap</span></div>
            </div>
                        ...
                        <>
                            <div className="flex items-center gap-4 mb-2">
                               ...
                               <span className="text-white text-lg">
                                   Is <span className="text-yellow-400 font-bold">{currentData.variables.compareVal1}</span> &lt; Pivot (<span className="text-red-500 font-bold">{currentData.variables.compareVal2}</span>)?
                               </span>
                            </div>
                            ...
                            {currentData.type === 'COMPARE' ? (
                              <div className="flex gap-4">
                                <button onClick={() => handlePracticeDecision('YES_SWAP')} className="px-6 py-4 bg-orange-600 hover:bg-orange-500 hover:scale-105 transition-all rounded-xl font-bold shadow-lg flex items-center gap-2"><ArrowRightLeft /> YES (Swap)</button>
                                <button onClick={() => handlePracticeDecision('NO_NEXT')} className="px-6 py-4 bg-slate-600 hover:bg-slate-500 hover:scale-105 transition-all rounded-xl font-bold shadow-lg flex items-center gap-2"><ArrowRight /> NO (Next)</button>
                              </div>
                            ) : (
                              <div className="flex flex-col items-center gap-2">
                                <p className="text-slate-400 italic">Processing Pivot/Swap...</p>
            ...
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl flex flex-col md:flex-row gap-6">
                 ...
                 <div className="w-full md:w-64 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="bg-slate-950 border border-slate-800 p-2 rounded flex justify-between items-center"><span className="text-slate-500 font-mono text-xs">Pivot</span><span className="text-red-500 font-mono font-bold">{currentData.variables.compareVal2 ?? '-'}</span></div>
                        <div className="bg-slate-950 border border-slate-800 p-2 rounded flex justify-between items-center"><span className="text-slate-500 font-mono text-xs">Current</span><span className="text-yellow-400 font-mono font-bold">{currentData.variables.compareVal1 ?? '-'}</span></div>
                    </div>
                 </div>
            </div>
        </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl flex flex-col flex-1 min-h-125">
                <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-950/50">
                    ...
                    <select value={selectedLanguage} onChange={(e) => setSelectedLanguage(e.target.value as Language)} className="bg-slate-800 text-slate-200 text-xs rounded px-2 py-1 border border-slate-700 focus:outline-none focus:border-purple-500">
                        {Object.keys(quickSortCode).map(lang => (<option key={lang} value={lang}>{lang}</option>))}
                    </select>
                </div>
                <div className="flex-1 p-4 overflow-auto bg-[#1e1e1e] font-mono text-xs md:text-sm relative">
                    <table className="w-full border-collapse">
                        <tbody>
                            {quickSortCode[selectedLanguage].code.split('\n').map((line, i) => (
