...
// Nội dung lý thuyết (English)
const BubbleSortTheory = () => (
  <div className="space-y-6 text-slate-300 leading-relaxed">
    <div>
      <h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2">
        <Info size={20} className="text-cyan-400" /> 1. What is Bubble Sort?
      </h3>
      <p>
        ...
        The name comes from the way smaller elements &quot;bubble&quot; to the top of the list (like air bubbles in water) while larger elements sink to the bottom.
      </p>
    </div>
    ...
    <div>
      ...
      <ul className="list-disc pl-5 space-y-2 marker:text-yellow-500">
        <li>Traverse from the left and compare adjacent elements and the one next to it.</li>
        <li>If they are in the wrong order (e.g., left &gt; right), <strong>swap</strong> them.</li>
        <li>Do this for every pair of adjacent elements to the end of the list.</li>
        <li>After each pass, the largest element is guaranteed to be at the correct position (sorted).</li>
      </ul>
    </div>
    ...
    <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
      ...
      <div className="font-mono text-sm space-y-1">
        <p><span className="text-yellow-400">Pass 1:</span> Compare 5 &amp; 1 &rarr; Swap &rarr; [1, 5, 4, 2]</p>
        <p className="pl-16">Compare 5 &amp; 4 &rarr; Swap &rarr; [1, 4, 5, 2]</p>
        <p className="pl-16">Compare 5 &amp; 2 &rarr; Swap &rarr; [1, 4, 2, <span className="text-green-400 font-bold">5</span>]</p>
        <p className="text-slate-500 italic mt-1">{`// 5 is now sorted at the end.`}</p>
        
        <p className="mt-2"><span className="text-yellow-400">Pass 2:</span> Compare 1 &amp; 4 &rarr; OK &rarr; [1, 4, 2, 5]</p>
        <p className="pl-16">Compare 4 &amp; 2 &rarr; Swap &rarr; [1, 2, <span className="text-green-400 font-bold">4, 5</span>]</p>
        <p className="text-slate-500 italic mt-1">{`// 4 is now sorted.`}</p>
      </div>
    </div>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-slate-800 p-3 rounded border border-slate-700">
          ...
          <p className="text-xs text-slate-500">Slow for large datasets (Nested loops).</p>
        </div>
        <div className="bg-slate-800 p-3 rounded border border-slate-700">
          ...
          <p className="text-xs text-slate-500">In-place sorting (No extra memory).</p>
        </div>
      </div>
    </div>
  </div>
...
export default function BubbleSortPage() {
  ...
  const [dataState] = useState(() => {
    ...
    return { arr, steps: generateBubbleSortTimeline(arr) };
  });
  ...
  const [userInput, setUserInput] = useState(dataState.arr.join(", "));
  ...
  const currentData = timeline[currentStep] || { 
    ...
    message: "Ready to start...", variables: { i: 0, j: 0 }, counts: { comparisons: 0, swaps: 0 }
  };
    ...
    <main className="flex min-h-screen flex-col items-center bg-slate-950 text-white p-4 md:p-8 relative">
            ...
            >
              ...
              <div className="flex justify-between items-center p-6 border-b border-slate-800 bg-slate-950">
                <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                  <BookOpen className="text-purple-400" /> Theory: Bubble Sort
                </h2>
      ...
      <div className="flex justify-between items-center w-full max-w-7xl mb-6">
        <button 
          ...
          className="group flex items-center gap-3 text-3xl font-bold bg-linear-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent hover:opacity-80 transition-opacity"
        >
          Bubble Sort Visualization
          <BookOpen className="text-slate-500 group-hover:text-purple-400 transition-colors" size={24} />
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 w-full max-w-7xl">
            ...
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl flex flex-col md:flex-row gap-6">
                 <div className="flex-1 space-y-2">
                    <div className="flex items-center gap-2 text-cyan-400 text-sm font-bold uppercase tracking-wider">
                        <Activity size={16} /> Status Log
                    </div>
                    <div className="bg-slate-950 border border-slate-800 rounded-lg p-4 h-24 flex items-center">
        ...
        <div className="xl:col-span-1 flex flex-col gap-6">
            <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl p-4 space-y-4">
               <div className="flex items-center gap-2 text-slate-200 font-bold pb-2 border-b border-slate-800">
