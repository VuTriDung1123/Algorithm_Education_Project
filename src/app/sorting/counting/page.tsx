...
import { playSuccessSound, playNote } from "@/lib/sound"; // Chỉ import cái cần dùng
...
function CountingSortVisualizer() {
  ...
  // Đã xóa các handler không dùng để clean code (StepBack, Skip, v.v...)
  ...
  // SỬA: Xóa biến decision không dùng
  const handlePracticeDecision = () => {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 w-full max-w-7xl">
        <div className="xl:col-span-2 space-y-6">
            <div className="flex flex-col gap-4">
                ...
                {/* SECONDARY VISUAL: COUNTING BOARD */}
                <div className="h-48 w-full">
                    <CountingBoard data={currentData} />
                </div>
            </div>
                ...
                {isPracticeMode ? (
                  <div className="flex flex-col items-center justify-center space-y-4 py-2">
                    {isFinished ? (
                        ...
                        <>
                            <div className="flex items-center gap-4 mb-2"><span className="text-purple-400 font-bold uppercase text-sm">Action:</span><span className="text-white text-lg">{currentData.message}</span></div>
                            <button onClick={() => handlePracticeDecision()} className="px-8 py-4 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold shadow-lg flex items-center gap-2"><ArrowDown /> EXECUTE STEP</button>
                        </>
                    )}
