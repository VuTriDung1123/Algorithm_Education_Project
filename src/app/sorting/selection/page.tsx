...
import SelectionSpotlight from "@/components/Visualization/SelectionSpotlight";
...
const SelectionSortTheory = () => (
  <div className="space-y-6 text-slate-300 leading-relaxed">
    <div>
      <h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2"><Info size={20} className="text-cyan-400" /> 1. What is Selection Sort?</h3>
      <p><strong>Selection Sort</strong> works by repeatedly selecting the smallest (or largest) element from the unsorted portion of the list and moving it to the sorted portion at the beginning.</p>
    </div>
    <div>
      ...
      <ul className="list-disc pl-5 space-y-2 marker:text-yellow-500">
        <li>Maintain two subarrays: one <strong>sorted</strong> and one <strong>unsorted</strong>.</li>
        <li>In every iteration, find the <strong>minimum element</strong> from the unsorted subarray.</li>
        <li>Swap it with the leftmost element of the unsorted subarray.</li>
        <li>Move the boundary of the sorted subarray one element to the right.</li>
      </ul>
    </div>
    <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700"><h3 className="text-white font-bold mb-2">Complexity</h3><p className="text-green-400 font-mono">Best/Avg/Worst: O(n²)</p><p className="text-xs text-slate-500">Inefficient on large lists.</p></div>
  </div>
);
...
function SelectionSortVisualizer() {
  ...
  useEffect(() => {
    ...
    else if (stepData.message.includes('New minimum')) playNote(600, 'sine', 0.1); // Sound effect khi tìm thấy Min mới
  ...
  // --- PRACTICE LOGIC ---
  const handlePracticeDecision = (decision: 'UPDATE_MIN' | 'NO_UPDATE' | 'SWAP') => {
    ...
    if (currentData.type === 'COMPARE') {
        const valJ = currentData.variables.compareVal1 || 0; // Số đang scan
        const currentMin = currentData.variables.compareVal2 || 0; // Số min hiện tại
        const shouldUpdate = valJ < currentMin;

        if (decision === 'UPDATE_MIN' && shouldUpdate) isCorrect = true;
        else if (decision === 'NO_UPDATE' && !shouldUpdate) isCorrect = true;
  ...
  const getBarColor = (index: number) => {
    ...
    if (index === variables.minIdx) return "bg-purple-600 shadow-[0_0_15px_rgba(147,51,234,0.7)] z-10"; // Highlight Min
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 w-full max-w-7xl">
        <div className="xl:col-span-2 space-y-6">
            <div className="flex flex-col gap-4">
                ...
                {/* 2. SPOTLIGHT VIEW (NEW) */}
                <div className="h-64 w-full">
                    <SelectionSpotlight data={currentData} array={currentData.arrayState} />
                </div>
            </div>
                            ...
                            {currentData.type === 'COMPARE' ? (
                                <div className="flex flex-col items-center gap-3">
                                    <div className="text-white text-lg text-center">
                                        Scan <span className="text-yellow-400 font-bold">{currentData.variables.compareVal1}</span> vs Current Min <span className="text-purple-400 font-bold">{currentData.variables.compareVal2}</span>
                                    </div>
                                    <div className="flex gap-4">
                                        <button onClick={() => handlePracticeDecision('UPDATE_MIN')} className="px-6 py-4 bg-purple-600 hover:bg-purple-500 rounded-xl font-bold">New Min!</button>
                                        <button onClick={() => handlePracticeDecision('NO_UPDATE')} className="px-6 py-4 bg-slate-600 hover:bg-slate-500 rounded-xl font-bold">No Change</button>
                                    </div>
                                </div>
                            ) : currentData.type === 'SWAP' ? (
                                <button onClick={() => handlePracticeDecision('SWAP')} className="px-8 py-4 bg-orange-600 hover:bg-orange-500 rounded-xl font-bold flex items-center gap-2"><ArrowRightLeft /> SWAP MIN TO START</button>
                            ) : (
            ...
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl flex flex-col md:flex-row gap-6">
                 ...
                 <div className="w-full md:w-64 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="bg-slate-950 border border-slate-800 p-2 rounded flex justify-between items-center"><span className="text-slate-500 font-mono text-xs">Scanner (j)</span><span className="text-white font-mono font-bold">{currentData.variables.j ?? '-'}</span></div>
                        <div className="bg-slate-950 border border-slate-800 p-2 rounded flex justify-between items-center"><span className="text-slate-500 font-mono text-xs">Current Min</span><span className="text-purple-400 font-mono font-bold">{currentData.variables.compareVal2 ?? '-'}</span></div>
                        <div className="bg-slate-950 border border-slate-800 p-2 rounded flex justify-between items-center"><span className="text-slate-500 font-mono text-xs">Min Index</span><span className="text-blue-400 font-mono font-bold">{currentData.variables.minIdx ?? '-'}</span></div>
                    </div>
                 </div>
            </div>
        </div>
        ...
        <div className="xl:col-span-1 flex flex-col gap-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl flex flex-col flex-1 min-h-125">
                <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-950/50">
                    ...
                    <select value={selectedLanguage} onChange={(e) => setSelectedLanguage(e.target.value as Language)} className="bg-slate-800 text-slate-200 text-xs rounded px-2 py-1 border border-slate-700 focus:outline-none focus:border-purple-500">{Object.keys(selectionSortCode).map(lang => (<option key={lang} value={lang}>{lang}</option>))}</select>
                </div>
                <div className="flex-1 p-4 overflow-auto bg-[#1e1e1e] font-mono text-xs md:text-sm relative">
                    <table className="w-full border-collapse"><tbody>{selectionSortCode[selectedLanguage].code.split('\n').map((line, i) => (<tr key={i} className={`relative ${getActiveLine() === i ? 'bg-yellow-500/20' : ''}`}><td className="w-8 text-right pr-4 text-slate-600 select-none border-r border-slate-700/50 align-top">{i + 1}</td><td className={`pl-4 align-top whitespace-pre-wrap ${getActiveLine() === i ? 'text-yellow-100 font-bold' : 'text-slate-300'}`}>{line}</td></tr>))}</tbody></table>
                </div>
            </div>
        </div>
      </div>
