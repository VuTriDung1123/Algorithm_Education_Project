...
import { generateShellSortTimeline } from "@/lib/algorithms/shellSort";
...
// --- THEORY ---
const ShellSortTheory = () => (
  <div className="space-y-6 text-slate-300 leading-relaxed">
    <div><h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2"><Info size={20} className="text-cyan-400" /> 1. What is Shell Sort?</h3><p><strong>Shell Sort</strong> is an optimization of Insertion Sort. It allows the exchange of items that are far apart. The idea is to arrange the list of elements so that, starting anywhere, taking every <em>h-th</em> element produces a sorted list.</p></div>
    <div><h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2"><Activity size={20} className="text-yellow-400" /> 2. How it works</h3><ul className="list-disc pl-5 space-y-2 marker:text-yellow-500"><li>Initialize the value of <code>gap</code> (e.g., n/2).</li><li>Divide the list into sub-lists of elements at distance <code>gap</code>.</li><li>Sort these sub-lists using <strong>Insertion Sort</strong>.</li><li>Reduce the <code>gap</code> (e.g., gap/2) and repeat until gap = 1.</li></ul></div>
    <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700"><h3 className="text-white font-bold mb-2">Complexity</h3><p className="text-green-400 font-mono">Avg: O(n log n) ~ O(n^1.5)</p><p className="text-red-400 font-mono">Worst: O(n²)</p><p className="text-xs text-slate-500">Depends on Gap Sequence.</p></div>
  </div>
);
...
function ShellSortVisualizer() {
  ...
  const loadNewArray = (newArr: number[]) => {
    const steps = generateShellSortTimeline(newArr);
  ...
  useEffect(() => {
    ...
    else if (stepData.type === 'SHIFT') playSwapSound();
    else if (stepData.type === 'INSERT') playNote(600, 'square', 0.05);
  ...
  const handlePracticeDecision = (decision: 'SHIFT' | 'INSERT') => {
    ...
    const valLeft = currentData.variables.compareVal1 || 0;
    const valTemp = currentData.variables.tempVal || 0;
    
    let isCorrect = false;
    const shouldShift = valLeft > valTemp;
    ...
    if (isCorrect) {
      ...
      setPracticeFeedback({ type: 'error', msg: 'Wrong! Compare with Temp.' });
      playErrorSound();
    }
  ...
  const getBarColor = (index: number) => {
    ...
    if (indices.includes(index)) {
        ...
        if (type === 'SHIFT') return "bg-orange-500";
  ...
  const getActiveLine = () => {
    const snippet = shellSortCode[selectedLanguage];
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-slate-950 text-white p-4 md:p-8 relative">
      <AnimatePresence>{isTheoryOpen && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={() => setIsTheoryOpen(false)}><motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-slate-900 border border-slate-700 w-full max-w-2xl max-h-[80vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}><div className="flex justify-between items-center p-6 border-b border-slate-800 bg-slate-950"><h2 className="text-2xl font-bold text-white flex items-center gap-2"><BookOpen className="text-purple-400" /> Theory: Shell Sort</h2><button onClick={() => setIsTheoryOpen(false)}><X size={24} /></button></div><div className="p-6 overflow-y-auto custom-scrollbar"><ShellSortTheory /></div></motion.div></motion.div>)}</AnimatePresence>
                    ...
                    {isFinished ? (
                        ...
                        <>
                            {currentData.type === 'COMPARE' && currentData.variables.i !== -1 ? (
                                <>
                                    <div className="flex items-center gap-4 mb-2"><span className="text-purple-400 font-bold uppercase text-sm">Gap: {currentData.variables.gap}</span><span className="text-white">Is <span className="text-yellow-400 font-bold">{currentData.variables.compareVal1}</span> &gt; Temp (<span className="text-pink-400 font-bold">{currentData.variables.tempVal}</span>)?</span></div>
                                ...
                                <div className="text-center">
                                    <p className="text-slate-400 mb-2 italic">Updating Gap...</p>
            ...
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl flex flex-col md:flex-row gap-6">
                 ...
                 <div className="w-full md:w-64 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div className="bg-slate-950 border border-slate-800 p-2 rounded flex justify-between items-center"><span className="text-slate-500 font-mono text-xs">Gap</span><span className="text-white font-mono font-bold">{currentData.variables.gap ?? '-'}</span></div>
                        <div className="bg-slate-950 border border-slate-800 p-2 rounded flex justify-between items-center"><span className="text-slate-500 font-mono text-xs">Temp (Key)</span><span className="text-pink-400 font-mono font-bold">{currentData.variables.tempVal ?? '-'}</span></div>
                        <div className="bg-slate-950 border border-slate-800 p-2 rounded flex justify-between items-center"><span className="text-slate-500 font-mono text-xs">Left Val</span><span className="text-yellow-400 font-mono font-bold">{currentData.variables.compareVal1 ?? '-'}</span></div>
                        <div className="bg-slate-950 border border-slate-800 p-2 rounded flex justify-between items-center"><span className="text-slate-500 font-mono text-xs">Index j</span><span className="text-blue-400 font-mono font-bold">{currentData.variables.j ?? '-'}</span></div>
                    </div>
                 </div>
            </div>
        </div>
