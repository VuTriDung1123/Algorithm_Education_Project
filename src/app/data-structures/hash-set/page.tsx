...
import { createEmptyHashTable, generateHashInsert, generateHashSearch, generateHashDelete } from "@/lib/data-structures/hashSetLogic";
import { hashSetSnippets } from "@/lib/data-structures/hashSetCode";
...
const HashTheory = () => (
  <div className="space-y-4 text-slate-300 text-sm">
    <p><strong>Hash Set (Bảng băm)</strong> là cấu trúc dữ liệu lưu trữ dưới dạng Key-Value, sử dụng hàm băm (Hash function) để tính toán index lưu trữ.</p>
...
function HashTableVisualizer() {
  ...
  const [activeCode, setActiveCode] = useState(hashSetSnippets.INSERT);
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-slate-950 text-white p-4 md:p-8 relative font-sans">
      ...
      <h1 className="text-3xl font-extrabold mb-2 bg-linear-to-r from-indigo-400 to-cyan-400 bg-clip-text text-transparent">Hash Set (Linear Probing)</h1>
                ...
                <div className="p-6 bg-slate-900/50 flex flex-wrap items-center justify-center gap-4 min-h-[100px]">
                    ...
                    {activeTab === 'INSERT' && (
                        <div className="flex items-center gap-3">
                            ...
                            <button onClick={() => { setActiveCode(hashSetSnippets.INSERT); runAnimation(generateHashInsert(arrayData, keyInput, valInput, MAX_CAPACITY)); }} disabled={isAnimating} className="ml-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded font-bold flex items-center gap-2"><Plus size={18}/> Insert</button>
                        </div>
                    )}
                    {activeTab === 'SEARCH' && (
                        <button onClick={() => { setActiveCode(hashSetSnippets.SEARCH); runAnimation(generateHashSearch(arrayData, keyInput, MAX_CAPACITY)); }} disabled={isAnimating} className="ml-2 px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded font-bold flex items-center gap-2"><Search size={18}/> Search</button>
                    )}
                    {activeTab === 'DELETE' && (
                        <button onClick={() => { setActiveCode(hashSetSnippets.DELETE); runAnimation(generateHashDelete(arrayData, keyInput, MAX_CAPACITY)); }} disabled={isAnimating} className="ml-2 px-6 py-2 bg-red-600 hover:bg-red-500 text-white rounded font-bold flex items-center gap-2"><Minus size={18}/> Delete</button>
                    )}
