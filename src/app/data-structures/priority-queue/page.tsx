...
import { createEmptyHeap, generateHeapInsert, generateHeapExtractMin } from "@/lib/data-structures/pqLogic";
import { pqSnippets } from "@/lib/data-structures/pqCode";

const MAX_CAPACITY = 15; // Phù hợp cho cây nhị phân hoàn chỉnh 4 tầng (1 + 2 + 4 + 8 = 15)

const HeapTheory = () => (
  <div className="space-y-4 text-slate-300 text-sm">
    <p><strong>Priority Queue (Hàng đợi ưu tiên)</strong> là một cây nhị phân hoàn chỉnh được lưu trữ dưới dạng mảng, trong đó giá trị của node cha luôn nhỏ hơn hoặc bằng giá trị các node con.</p>
...
function MinHeapVisualizer() {
  ...
  const activeCodeSnippet = pqSnippets[activeCodeContext] || pqSnippets.INSERT;
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-slate-950 text-white p-4 md:p-8 relative font-sans">
      ...
      <h1 className="text-3xl font-extrabold mb-2 bg-linear-to-r from-amber-400 to-orange-500 bg-clip-text text-transparent">Priority Queue (Priority Queue)</h1>
