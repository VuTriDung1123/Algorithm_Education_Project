...
import { createEmptyHeap, generateHeapInsert, generateHeapExtractMin } from "@/lib/data-structures/maxHeapLogic";
import { maxHeapSnippets } from "@/lib/data-structures/maxHeapCode";

const MAX_CAPACITY = 15; // Phù hợp cho cây nhị phân hoàn chỉnh 4 tầng (1 + 2 + 4 + 8 = 15)

const HeapTheory = () => (
  <div className="space-y-4 text-slate-300 text-sm">
    <p><strong>Max Heap (Hàng đợi ưu tiên)</strong> là một cây nhị phân hoàn chỉnh được lưu trữ dưới dạng mảng, trong đó giá trị của node cha luôn nhỏ hơn hoặc bằng giá trị các node con.</p>
...
function MinHeapVisualizer() {
  ...
  const activeCodeSnippet = maxHeapSnippets[activeCodeContext] || maxHeapSnippets.INSERT;
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-slate-950 text-white p-4 md:p-8 relative font-sans">
      ...
      <h1 className="text-3xl font-extrabold mb-2 bg-linear-to-r from-amber-400 to-orange-500 bg-clip-text text-transparent">Max Heap (Priority Queue)</h1>
