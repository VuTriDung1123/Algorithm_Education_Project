...
import { 
  ...
  Trash2, Search as SearchIcon, Activity, Zap, ArrowRightLeft
} from "lucide-react";
import Link from "next/link";
import { DSAnimationStep, ArrayNode } from "@/lib/data-structures/types"; 
import { doublyLinkedListSnippets } from "@/lib/data-structures/doublyLinkedListCode";
import { 
    initDoublyLinkedList,
    generateDLLSearch, 
    generateDLLInsertHead, 
    generateDLLInsertTail, 
    generateDLLInsertIndex,
    generateDLLDeleteIndex 
} from "@/lib/data-structures/doublyLinkedListLogic";
import DoublyLinkedListNode from "@/components/Visualization/DoublyLinkedList/DoublyLinkedListNode";

// --- THEORY CONTENT ---
const DoublyLinkedListTheory = () => (
  <div className="space-y-6 text-slate-300 leading-relaxed text-sm md:text-base">
    <section>
        <h3 className="text-xl font-bold text-white mb-3 flex items-center gap-2">
            <ArrowRightLeft size={20} className="text-purple-400"/> 1. Danh sách liên kết đôi (Doubly Linked List)
        </h3>
        <p className="mb-2">
            Mở rộng từ danh sách liên kết đơn, mỗi nút (Node) trong danh sách liên kết đôi chứa 3 thành phần:
        </p>
        <ul className="list-disc pl-5 space-y-1 marker:text-purple-500">
            <li><strong>Data:</strong> Giá trị lưu trữ.</li>
            <li><strong>Next Pointer:</strong> Trỏ đến nút tiếp theo.</li>
            <li><strong>Prev Pointer:</strong> Trỏ về nút phía trước nó.</li>
        </ul>
    </section>
    <div className="w-full h-px bg-slate-700/50 my-4"></div>
    <section>
        <h3 className="text-xl font-bold text-white mb-3 flex items-center gap-2">
            <Zap size={20} className="text-yellow-400"/> 2. Ưu điểm & Độ phức tạp
        </h3>
        <p className="mb-4 text-sm text-slate-400">Có thể duyệt theo cả 2 chiều. Xóa một nút dễ dàng hơn nếu đã biết địa chỉ của nó vì không cần duyệt lại từ đầu để tìm nút `Prev`.</p>
        <div className="grid grid-cols-2 gap-4 text-sm">
            <div className="bg-slate-950 p-3 rounded border border-slate-800">
                <span className="block text-slate-500 text-xs font-bold uppercase">Search</span>
                <span className="text-yellow-400 font-mono font-bold text-lg">O(N)</span>
            </div>
            <div className="bg-slate-950 p-3 rounded border border-slate-800">
                <span className="block text-slate-500 text-xs font-bold uppercase">Insert/Delete (Head/Tail)</span>
...
const TabBtn = ({ name, isActive, onClick }: TabBtnProps) => (
    <button onClick={onClick} className={`flex-1 px-4 py-3 font-bold text-xs md:text-sm transition-all border-b-2 whitespace-nowrap ${isActive ? 'border-purple-500 text-purple-400 bg-slate-900/50' : 'border-transparent text-slate-500 hover:text-slate-300 hover:bg-slate-900/20'}`}>{name}</button>
);

export default function DoublyLinkedListPage() {
    return (
        <Suspense fallback={<div className="p-10 text-center text-white">Loading Doubly Linked List...</div>}>
...
function DoublyLinkedListVisualizer() {
  ...
  const [activeCode, setActiveCode] = useState(doublyLinkedListSnippets.INSERT_TAIL);
  const [isTheoryOpen, setIsTheoryOpen] = useState(false);
  const [valInput, setValInput] = useState(88);
  const [idxInput, setIdxInput] = useState(0);

  useEffect(() => {
      const initTimer = setTimeout(() => {
          setIsMounted(true);
          setNodes(initDoublyLinkedList([10, 20, 30]));
      }, 0);
  ...
  const handleSearch = () => { setActiveCode(doublyLinkedListSnippets.SEARCH); runAnimation(generateDLLSearch(nodes, valInput)); };
  const handleInsertHead = () => { setActiveCode(doublyLinkedListSnippets.INSERT_HEAD); runAnimation(generateDLLInsertHead(nodes, valInput)); };
  const handleInsertTail = () => { setActiveCode(doublyLinkedListSnippets.INSERT_TAIL); runAnimation(generateDLLInsertTail(nodes, valInput)); };
  const handleInsertIndex = () => {
      if (idxInput < 0 || idxInput > nodes.length) return alert("Invalid Index");
      setActiveCode(doublyLinkedListSnippets.INSERT_INDEX);
      runAnimation(generateDLLInsertIndex(nodes, idxInput, valInput));
  };
  const handleDeleteIndex = () => {
      if (idxInput < 0 || idxInput >= nodes.length) return alert("Invalid Index");
      if (idxInput === 0) setActiveCode(doublyLinkedListSnippets.DELETE_HEAD);
      else if (idxInput === nodes.length - 1) setActiveCode(doublyLinkedListSnippets.DELETE_TAIL);
      else setActiveCode(doublyLinkedListSnippets.DELETE_INDEX);
      runAnimation(generateDLLDeleteIndex(nodes, idxInput));
  };

  if (!isMounted) return <div className="flex min-h-screen items-center justify-center bg-slate-950 text-slate-500 font-mono">Initializing Memory Addresses...</div>;
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-slate-950 text-white p-4 md:p-8 relative font-sans">
            ...
            <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={()=>setIsTheoryOpen(false)}>
                <motion.div initial={{scale:0.9, opacity:0}} animate={{scale:1, opacity:1}} exit={{scale:0.9, opacity:0}} className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden" onClick={(e)=>e.stopPropagation()}>
                    <div className="flex justify-between items-center p-6 border-b border-slate-800 bg-slate-950">
                        <h2 className="text-2xl font-bold text-white flex items-center gap-2"><BookOpen className="text-purple-400" /> Theory</h2>
                        <button onClick={()=>setIsTheoryOpen(false)}><X size={24} className="text-slate-400 hover:text-white"/></button>
                    </div>
                    <div className="p-6 overflow-y-auto custom-scrollbar"><DoublyLinkedListTheory/></div>
                </motion.div>
            </motion.div>
        )}
      </AnimatePresence>
      ...
      <h1 className="text-3xl font-extrabold mb-8 bg-linear-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">Doubly Linked List</h1>
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 relative overflow-hidden min-h-87.5 shadow-xl flex flex-col justify-center">
                <div className="absolute top-4 left-4 flex gap-4 text-xs font-mono text-slate-500 uppercase tracking-widest">
                    <span>Size: <strong className="text-purple-400">{currentNodes.length}</strong></span>
                </div>
                    ...
                    <div className="flex items-start min-w-max px-4">
                        <AnimatePresence mode="popLayout">
                            {currentNodes.map((node: ArrayNode, i: number) => (
                                ...
                                >
                                    <DoublyLinkedListNode node={node} isHead={i === 0} isTail={i === currentNodes.length - 1} />
                                </motion.div>
                            ))}
                        </AnimatePresence>
                        {currentNodes.length === 0 && <div className="text-slate-600 italic text-sm p-4">List is empty (Head = NULL)</div>}
                    </div>
                </div>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
                ...
                <div className="p-6 bg-slate-900/50 flex flex-wrap items-center gap-4 min-h-25">
                    {activeTab === 'INSERT' && (
                        <>
                            <div className="flex items-center gap-2 p-2 bg-slate-950 rounded border border-slate-800"><span className="text-xs font-bold text-slate-500 uppercase">Val</span><input type="number" value={valInput} onChange={(e) => setValInput(Number(e.target.value))} className="w-12 bg-transparent text-white text-center focus:outline-none font-mono" /></div>
                            <button onClick={handleInsertHead} disabled={isAnimating} className="btn-secondary hover:border-purple-500">Head</button>
                            <button onClick={handleInsertTail} disabled={isAnimating} className="btn-secondary hover:border-purple-500">Tail</button>
                            <div className="h-8 w-px bg-slate-700 mx-2 hidden md:block"></div>
                            <div className="flex items-center gap-2 p-2 bg-slate-950 rounded border border-slate-800"><span className="text-xs font-bold text-slate-500 uppercase">Idx</span><input type="number" value={idxInput} onChange={(e) => setIdxInput(Number(e.target.value))} className="w-10 bg-transparent text-white text-center focus:outline-none font-mono" /></div>
                            <button onClick={handleInsertIndex} disabled={isAnimating} className="btn-primary bg-purple-600 hover:bg-purple-500">Insert(i)</button>
                        </>
                    )}
                    {activeTab === 'DELETE' && (
                        <>
      ...
      <style jsx>{`
        ...
        .btn-secondary { @apply px-4 py-2 bg-slate-800 border border-slate-700 text-slate-200 rounded font-bold transition-all active:scale-95 disabled:opacity-50 disabled:pointer-events-none; }
      `}</style>
    </main>
  );
}