...
export function* generateHeapExtractMin(
    ...
    if (newSize > 0) {
        ...
        while (true) {
            ...
            if (left < newSize && arr[left].value! < arr[smallest].value!) {
                smallest = left;
            }
            if (right < newSize && arr[right].value! < arr[smallest].value!) {
                smallest = right;
            }
            ...
            arr[index].state = 'SELECTED';
            arr[smallest].state = 'ACCESS';
            yield { arrayState: JSON.parse(JSON.stringify(arr)), message: `Smallest child is ${arr[smallest].value} at index ${smallest}. Swapping...`, codeLine: 12, auxiliary: { code: 'HEAPIFY_DOWN', size: newSize } };

            // Swap
            let temp = arr[index].value;
            arr[index].value = arr[smallest].value;
            arr[smallest].value = temp;
            
            let tempId = arr[index].id;
            arr[index].id = arr[smallest].id;
            arr[smallest].id = tempId;

            yield { arrayState: JSON.parse(JSON.stringify(arr)), message: `Swapped.`, codeLine: 13, auxiliary: { code: 'HEAPIFY_DOWN', size: newSize } };

            arr[index].state = 'DEFAULT';
            arr[smallest].state = 'DEFAULT';
            index = smallest;
        }
    }
