...
const algos = [
    { id: 'linear', name: 'LinearSearch', title: 'Linear Search' },
    { id: 'binary', name: 'BinarySearch', title: 'Binary Search' },
    { id: 'ternary', name: 'TernarySearch', title: 'Ternary Search' },
    { id: 'interpolation', name: 'InterpolationSearch', title: 'Interpolation Search' },
    { id: 'jump', name: 'JumpSearch', title: 'Jump Search' },
    { id: 'exponential', name: 'ExponentialSearch', title: 'Exponential Search' }
];

if (!fs.existsSync('src/lib/search')) {
    fs.mkdirSync('src/lib/search', { recursive: true });
}

algos.forEach(algo => {
    // 1. Code Snippet
    const codeContent = `export const ${algo.name.charAt(0).toLowerCase() + algo.name.slice(1)}Snippets = {
  javascript: \`function ${algo.name.charAt(0).toLowerCase() + algo.name.slice(1)}(arr, target) {
  // TODO
}\`,
  python: \`def ${algo.name.toLowerCase()}_search(arr, target):
  # TODO\`,
  cpp: \`int ${algo.name.toLowerCase()}Search(vector<int>& arr, int target) {
  // TODO
}\`
};`;
    fs.writeFileSync(`src/lib/search/${algo.id}SearchCode.ts`, codeContent);

    // 2. Logic
    const logicContent = `import { DSAnimationStep, SearchStateData } from "@/lib/data-structures/types";

export function* ${algo.name.charAt(0).toLowerCase() + algo.name.slice(1)}Logic(arr: number[], target: number): Generator<DSAnimationStep> {
    const searchState: SearchStateData = {
        array: [...arr],
        target,
        left: 0,
        right: arr.length - 1
    ...
    yield {
        ...
        message: 'Bắt đầu tìm kiếm ' + target,
        codeLine: 1
    };
    ...
    fs.writeFileSync(`src/lib/search/${algo.id}SearchLogic.ts`, logicContent);

    // 3. Page
    const pageDir = `src/app/search/${algo.id}`;
    if (!fs.existsSync(pageDir)) fs.mkdirSync(pageDir, { recursive: true });
    const fnName = algo.name.charAt(0).toLowerCase() + algo.name.slice(1);
...
import { ${fnName}Logic } from "@/lib/search/${algo.id}SearchLogic";
import { ${fnName}Snippets } from "@/lib/search/${algo.id}SearchCode";
import { DSAnimationStep, SearchStateData } from "@/lib/data-structures/types";

export default function ${algo.name}Page() {
    ...
    const startSearch = () => {
        if (isPlaying) return;
        generatorRef.current = ${fnName}Logic(array, target);
        setIsPlaying(true);
        executeNextStep();
    };
    ...
    return (
        <main className="flex min-h-screen flex-col items-center bg-slate-950 text-white p-4 font-sans">
            ...
            <div className="w-full max-w-7xl flex items-center justify-between mb-6">
                ...
                <h1 className="text-3xl font-bold">${algo.title}</h1>
            ...
            <div className="w-full max-w-7xl mt-12 bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
                ...
                <div className="p-4 font-mono text-sm overflow-x-auto">
                    {${fnName}Snippets.javascript.split('\\n').map((line: string, i: number) => (
    ...
    fs.writeFileSync(pageDir + '/page.tsx', pageContent);
});
