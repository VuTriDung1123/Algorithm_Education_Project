...
def generate_page(algo):
    defaultArgs = ''
    if algo['libFile'] == 'fibonacci': defaultArgs = '6'
    elif algo['libFile'] == 'lcs': defaultArgs = '"AGGTAB", "GXTXAYB"'
    elif algo['libFile'] == 'knapsack': defaultArgs = '[1, 2, 3], [10, 15, 40], 6'
    elif algo['libFile'] == 'coinChange': defaultArgs = '[1, 2, 5], 11'
    elif algo['libFile'] == 'lis': defaultArgs = '[10, 9, 2, 5, 3, 7, 101, 18]'
    elif algo['libFile'] == 'editDistance': defaultArgs = '"horse", "ros"'
    elif algo['libFile'] == 'mcm': defaultArgs = '[1, 2, 3, 4]'
    elif algo['libFile'] == 'subsetSum': defaultArgs = '[3, 34, 4, 12, 5, 2], 9'

    funcName = algo['libFile'][0].upper() + algo['libFile'][1:]
...
if not os.path.exists('src/lib/search'):
    os.makedirs('src/lib/search')

for algo in dpAlgorithms:
    with open(f"src/lib/search/{algo['libFile']}Code.ts", "w", encoding="utf-8") as f:
        f.write(algo['code'])
    
    with open(f"src/lib/search/{algo['libFile']}Logic.ts", "w", encoding="utf-8") as f:
        f.write(algo['logic'])

    pageDir = f"src/app/dp/{algo['id']}"
    if not os.path.exists(pageDir):
        os.makedirs(pageDir)
    
    with open(f"{pageDir}/page.tsx", "w", encoding="utf-8") as f:
        f.write(generate_page(algo))
