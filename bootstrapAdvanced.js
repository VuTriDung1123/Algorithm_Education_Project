const fs = require('fs');
const algos = ['segment-tree', 'fenwick', 'rbt', 'splay', 'treap', 'b-tree', 'b-plus-tree'];
algos.forEach(a => {
  const dir = 'src/app/data-structures/'+a;
  if(!fs.existsSync(dir)) fs.mkdirSync(dir, {recursive: true});
  const Name = a.split('-').map(w=>w[0].toUpperCase()+w.slice(1)).join(' ');
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-slate-950 text-white p-4 font-sans relative">
      ...
      <div className="bg-slate-900 border border-slate-800 p-12 rounded-3xl flex flex-col items-center text-center shadow-2xl max-w-lg">
        ...
        <h1 className="text-3xl font-extrabold mb-4 bg-linear-to-r from-yellow-400 to-orange-500 bg-clip-text text-transparent">
            ${Name}
        </h1>
        <p className="text-slate-400 leading-relaxed">
            Thuật toán này thuộc nhóm cấu trúc dữ liệu Nâng cao (Advanced) và đang trong quá trình xây dựng thuật toán mô phỏng chi tiết. 
            <br/><br/>
